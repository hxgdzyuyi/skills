...
- 正文里**不要用 `<div>`**——公众号编辑器会把粘贴进来的 `<div>` 统一替换成 `<section>`，过程中可能丢失或改写其上的内联样式。需要块级容器一律直接写 `<section>`（或 `<p>`）。
...
> 实测补充：除以上几项外，公众号**会保留大量内联 CSS**，但保存后会二次清洗/序列化，复杂布局属性不一定稳定。`transform`、`float`、`position`、`grid`、`background-image`、`box-shadow`、`opacity` 在真实推文里常见；`display:flex` 可用，但只能按下面的有限规则使用。

## Flex / 多列布局规则

公众号文章里的 `display:flex` 适合做**简单的一行内对齐**，不适合做让编辑器重新计算的响应式网格。粘贴时正常、保存后变形，常见原因是微信保留了父级 `display:flex`，但删掉了子项上的 `flex` 计算规则，导致卡片按内容宽度收缩。

- 可以用：`display:-webkit-flex;display:flex;justify-content:center|space-between;align-items:center|stretch;`
- 可以用：子项显式 `width:48%;max-width:48%;box-sizing:border-box;`，父级一行只放两个子项。
- 避免用：`flex:1`、`flex:1 1 ...`、`flex-basis`、`flex-wrap`、`gap`、`calc()` 参与列宽计算。
- 多卡片网格不要写一个 `flex-wrap` 容器；改成多行容器，每行一个 flex 父级，每行两个固定宽度子块。
- 徽章、胶囊按钮、居中提示等不需要 flex；优先用 `text-align:center`、`display:inline-block`、固定 `padding`。
...
- 多列卡片按“每行一个 flex 父级 + 固定宽度子项”生成；不要输出 `flex-wrap`/`gap`/`flex:...`/`calc()` 组合。
...
- `display:flex` 只用于简单行内对齐。`flex-wrap + gap + flex:1 1 calc(...)` 这类网格粘贴时可能正常，保存后会被微信删掉子项 `flex`，造成间距和宽度错乱。
