...
#
# Usage:
#   scripts/skill.sh install   <codex|claude> <skill-name> [-p <project-dir>]
...
usage() {
    cat <<EOF
Usage:
  $(basename "$0") install   <codex|claude> <skill-name> [-p <project-dir>]
...
# Parse a trailing/embedded -p|--project <dir> option; collect the rest as positionals.
project=""
positional=()
while [[ $# -gt 0 ]]; do
    case "$1" in
...
case "$action" in
    install)   [[ $# -eq 2 ]] || usage; cmd_install "$1" "$2" "$project" ;;
